...
/*
La fonction getAllGroupsByUserId($id) rŽcupre un array des numŽros des groupes auxquel $id appartient.

La fonction checkPermission($vargroup, $user) renvoie TRUE si le $user appartient ˆ au moins un des groupes $vargroup. 
	...
	if(!isLost()){
		if (isset($_SESSION['id'])) { // if logged in
			if (isLoggedVisitor()) { // if visitor 
				// Requte qui rŽcupre toutes les coordonnŽes du client
...
if (isset($userid)){  // vŽrification si loguŽ en tant qu'utilisateur
	...
	//requte pour recupŽrer les ingrŽdients
