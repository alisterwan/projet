...
<option value='Zimbabwe'>Zimbabwe</option>
	...
	if ($_POST) {
		...
		else if (pg_num_rows(pg_query($conn,"SELECT username from users where username='$user'"))) {
		...
		else if (pg_num_rows(pg_query($conn,"SELECT mail from users where mail='$email'"))) {
		...
		// Ajout d'un nouveau client dans la base de donnée
			...
			$pass = sha1($pass);
			if (!pg_query($conn,"INSERT INTO users(firstname, surname, address, city, country, username, password, mail) VALUES ('$firstname','$surname','$address','$city','$country','$user','$pass','$email')")) {
...
/*
		...
		if (mail($to,"Registration to LMLV","You have been correctly registered.", $headers))
			$message = "<p>An email of confirmation has been sent.</p>";
		else {
			$message = "<p class='error'>The email has not been sent.</p>";
			printForm($firstname,$surname,$address,$city,$country,$user,$email);
		}
*/
	}
